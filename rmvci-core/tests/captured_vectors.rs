...
#[test]
fn parse_challenge() {
    let chal = [0x0e, 0x00, 0x09, 0x00, 0x01, 0xb0, 0xcb, 0x49, 0x68, 0x07, 0x45, 0xc8, 0x7f, 0xa9];
...
fn set_config_7_0_wire_old_key() {
    let exp = [
        0x13, 0x00, 0x1a, 0x7c, 0xef, 0xa7, 0x56, 0x16, 0x8c, 0xbc, 0x3f, 0x7d, 0x9a, 0x06, 0x8e,
        0x58, 0x87, 0xe6, 0x24,
    ];
...
fn set_config_7_1_wire_old_key() {
    let exp = [
        0x13, 0x00, 0x1a, 0x7c, 0xef, 0xa7, 0x56, 0x16, 0x8c, 0xbc, 0x0a, 0xbd, 0x17, 0x8f, 0xd9,
        0x48, 0xcf, 0x57, 0x6b,
    ];
...
fn set_config_7_0_wire_new_key() {
    ...
    let body = [
        0x6f, 0xa4, 0xaf, 0x0b, 0x4b, 0x42, 0xd6, 0x95, 0xc8, 0x6c, 0x26, 0xf5, 0x7b, 0x63, 0xd4,
        0x94,
    ];
...
fn start_filter_iso15765_flow_control() {
    let exp = [
        0x19, 0x00, 0x0b, 0x06, 0, 0, 0, 0x8b, 0x7a, 0x0e, 0x00, 0x03, 0, 0, 0, 0xff, 0xff, 0xff,
        0xff, 0x00, 0x00, 0x07, 0xcc, 0x00, 0x00, 0x07, 0xc4,
    ];
...
fn write_msg_29bit_txflags() {
    ...
    let exp = [0x0f, 0, 0x0a, 0x06, 0, 0, 0, 0x00, 0x01, 0, 0, 0x00, 0x00, 0x07, 0xc4, 0x21, 0x43];
...
fn inner_connect_iso14230() {
    let exp = [0x0d, 0x00, 0x07, 0x04, 0, 0, 0, 0, 0x10, 0, 0, 0xa0, 0x28, 0, 0];
...
fn inner_start_filter_kline() {
    let exp = [0x10, 0, 0x0b, 0x04, 0, 0, 0, 0x8b, 0x7a, 0x0e, 0, 0x01, 0, 0, 0, 0xc0, 0xc0, 0];
    let got = inner::start_filter(ProtocolId::Iso14230, 0x000e_7a8b, 1, &[0xc0], &[0xc0], None, 1)
        .unwrap();
    assert_eq!(got, exp);
}
...
fn inner_write_msg_kline() {
    let exp = [0x0e, 0, 0x0a, 0x04, 0, 0, 0, 0, 0, 0, 0, 0x82, 0x19, 0xf0, 0x01, 0x05];
...
fn parse_read_reply_kline() {
    let reply = [
        0x0f, 0, 0x09, 0, 0, 0, 0, 0, 0, 0, 0, 0x83, 0xf0, 0x19, 0x41, 0x0d, 0, 0, 0, 0, 0, 0, 0, 0,
    ];
    let parsed = inner::parse_read_reply(&reply).unwrap();
...
fn connect_full_frame_new_key() {
    let cipher = [
        0x5b, 0x81, 0x15, 0x2c, 0xcb, 0xe7, 0xd9, 0xc2, 0x47, 0x5c, 0xeb, 0x5c, 0x21, 0x70, 0xf0,
        0xdd,
    ];
